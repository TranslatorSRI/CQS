...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let test_case_dir_files = fs::read_dir(test_case_dir.as_path()).expect("reading contents of test cases dir");
    ...
    let files = match options.limit {
        Some(l) => test_case_files.into_iter().take(l as usize).collect_vec(),
        None => test_case_files,
    };

    for test_case_file in files {
